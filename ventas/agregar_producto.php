...
<script>
function agregarProducto(codigo_producto, nombre_producto, categoria_producto, precio_c, precio_v, proveedor){
          if ($('#nombre_producto').val().trim() === '') {
            $("#resultado").html("El campo nombre esta vacio");
          }else{
            if ($('#precio_c').val().trim() === '') {
              $("#resultado").html("El campo precio de compra esta vacio");
            }else{
              if ($('#precio_v').val().trim() === '') {
                $("#resultado").html("El campo precio de venta esta vacio");
              }else{
                if ($('#proveedor').val().trim() === '') {
                  $("#resultado").html("El campo proveedor esta vacio");
                }else{
                  if ($('#codigo_producto').val().trim() === '') {
                  $("#resultado").html("El codigo del producto esta vacio");
                }else{
                  var parametros = {
        ...
        $.ajax({
                ...
                url:   'php/add_producto.php', //archivo que recibe la peticion
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li><a href="index.php"><i class="fa fa-dashboard"></i> <span>Inicio</span></a></li>
          ...
          <ul class="treeview-menu">
            ...
            <li class="active"><a href="#"><i class="fa fa-circle-o"></i> Agregar Productos</a></li>
            ...
            <form action="#" method="post" id="agregar_producto">
              <div class="box-body">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" placeholder="Nombre Producto" id="nombre_producto" required="">
                </div>
                <div class="form-group">
                  <label for="precio_venta">Categoría</label>
                  <input type="text" class="form-control" placeholder="Categoría" id="categoria_producto" required="">
                </div>
                <div class="form-group">
                  <label for="precio_compra">Precio Compra</label>
                  <input type="text" class="form-control" placeholder="Precio Compra" id="precio_c" required="">
                </div>
                <div class="form-group">
                  <label for="precio_venta">Precio Venta</label>
                  <input type="text" class="form-control" placeholder="Precio Venta" id="precio_v" required="">
                </div>
                <div class="form-group">
                  ...
                  <select id="proveedor" class="form-control">
                    ...
                    <?php
                    $q = mysql_query("SELECT * FROM proveedores");
                    while ($row=mysql_fetch_array($q)) {
                      echo "<option value='$row[id_prove]'>$row[nombre_prove]</option>";
                    }
                    ?>
                  </select>
                </div>
                <div class="form-group">
                  <label for="codigo_producto"><h4>Codigo Producto</h4></label>
                  <input type="text" class="form-control" placeholder="Codigo Producto" id="codigo_producto" required="">
                </div>
              </div>
              <!-- /.box-body -->

              <div class="box-footer">
                <input type="submit" href="javascript:;" onclick="agregarProducto($('#codigo_producto').val(), $('#nombre_producto').val(), $('#categoria_producto').val(), $('#precio_c').val(), $('#precio_v').val(), $('#proveedor').val());return false;" value="Enviar" class="btn btn-primary"/>
              </div>
            </form>
          </div>
          <!-- /.box -->
